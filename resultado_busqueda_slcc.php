...
try {
    if (isset($_POST['tipo_busqueda_slcc']) && isset($_POST['termino_busqueda_slcc'])) {
        $tipoBusqueda = $_POST['tipo_busqueda_slcc'];
        $terminoBusqueda = $_POST['termino_busqueda_slcc'];
        $campoBusqueda = '';

        if ($tipoBusqueda === 'hechos_esclarecidos_slcc' && isset($_POST['hechos_busqueda_slcc'])) {
            $campoBusqueda = $_POST['hechos_busqueda_slcc'];
            $sql = "SELECT id, n_sumario, dep_instructora, caratula, magis_interventor, num_sum_relacionado, cant_allanamientos, cant_aut_detenidos FROM provisorio.hechos_esclarecidos_slcc WHERE LOWER($campoBusqueda) LIKE LOWER(:terminoBusqueda)";
        } elseif ($tipoBusqueda === 'detenidos_slcc' && isset($_POST['detenidos_busqueda_slcc'])) {
            $campoBusqueda = $_POST['detenidos_busqueda_slcc'];
            $sql = "SELECT id, n_sumario, dep_instructora, dni, apellido, nombre, fecha_nac, edad, genero, nacionalidad, domicilio FROM provisorio.detenidos_slcc WHERE LOWER($campoBusqueda) LIKE LOWER(:terminoBusqueda)";
        } else {
        ...
        if (is_array($resultados) && count($resultados) > 0) {
            ...
            } elseif ($tipoBusqueda === 'detenidos_slcc') {
                echo "<th>ID</th>";
                echo "<th>Núm. Sumario</th>";
