...
try {
    if (isset($_POST['consulta_sii'])) {
        $consulta = $_POST['consulta_sii'];

        if ($consulta === 'hechos_esclarecidos_sii') {
            $sql = "SELECT id, n_sumario, dep_instructora, caratula, magis_interventor, num_sum_relacionado, cant_allanamientos, cant_aut_detenidos FROM provisorio.hechos_esclarecidos_sii";
        } elseif ($consulta === 'detenidos_sii') {
            $sql = "SELECT n_sumario, dni, apellido, nombre, fecha_nac, edad, genero, nacionalidad, domicilio FROM provisorio.detenidos_sii";
        } else {
